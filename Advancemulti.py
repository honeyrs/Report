...
# Ask for API ID, API hash, and user sessions at runtime
api_id = input(colored("Enter your Telegram API ID: ", "blue"))
api_hash = input(colored("Enter your Telegram API Hash: ", "blue"))

# Allow the user to input multiple user sessions
user_sessions_input = input(colored("Enter the user session files (comma separated, e.g., user_session1,user_session2): ", "blue"))
user_sessions = [session.strip() for session in user_sessions_input.split(",")]

# Ask for the delay between reports
delay_time = get_valid_integer_input(colored("Enter the delay between reports in seconds (e.g., 5): ", "blue"))
...
async def report_entity(client, delay_time):
    while True:
            ...
            while True:
                ...
                try:
                    ...
                    for i in range(num_reports):
                        ...
                        time.sleep(delay_time)  # Delay between reports
...
def main():
    for user_session in user_sessions:
        ...
        client.start()
        print(f"Session {user_session} started.")
        client.loop.run_until_complete(report_entity(client, delay_time))
